# -*- coding: utf-8 -*-
"""
Created on Mon Jul 22 22:54:54 2019
...
# work director
os.chdir(r'F:/ZINDI/Fraud_Detection')
...
# Data splite
ytrain = data['FraudResult']
xtrain=data.drop(['FraudResult', 'TransactionId'], axis=1)
...
#Transformation variable to catogory
xtrain['BatchId']=xtrain["BatchId"].astype('category')
xtrain['AccountId']=xtrain["AccountId"].astype('category') 
xtrain['SubscriptionId']=xtrain["SubscriptionId"].astype('category')  
xtrain['CustomerId']=xtrain["CustomerId"].astype('category')
xtrain=xtrain.drop(['CurrencyCode', 'CountryCode'], axis=1)
xtrain['ProviderId']=xtrain["ProviderId"].astype('category')
xtrain['ProductId']=xtrain["ProductId"].astype('category')
xtrain['ChannelId']=xtrain["ChannelId"].astype('category')
...
plt.xlabel('fraude result categories')
plt.xticks(range(2), np.unique(ytrain),rotation='vertical')
plt.show()
